...
function startMessage(chatId, ctx) {
...
如遇問題請重新使用 \`/start\` 指令`,
    {
      ...
      reply_markup: {
        ...
        inline_keyboard: [
          [{ text: 'Check duty', callback_data: 'check' }],
          [{ text: 'Set duty', callback_data: 'set' }],
        ],
      },
    }
  );
...
bot.action('start', ctx => {
  startMessage(ctx.chat.id, ctx);
});

bot.action('check', ctx => {
  ctx.editMessageText('Checking duty...', {
    reply_markup: {
      inline_keyboard: [[{ text: 'Back to start', callback_data: 'start' }]],
    },
  });
});
...
//   let duty;
//   bot.telegram.sendChatAction(ctx.chat.id, 'typing');
//   axios.get(process.env.DUTYFILE).then(response => {
